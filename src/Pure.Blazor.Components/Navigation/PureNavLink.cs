...
///
/// Addresses compatibility issues with Tailwind styling:
/// https://github.com/dotnet/aspnetcore/issues/37765
///
/// ---
/// A component that renders an anchor tag, automatically toggling its 'active'
/// class based on whether its 'href' matches the current URI.
/// </summary>
public class PureNavLink : ComponentBase, IDisposable
{
    ...
    private void UpdateCssClass()
    {
        ...
        if (_isActive)
        {
            activeStateClass = ActiveClass ?? DefaultActiveClass;
        }
        else
        {
            activeStateClass = InactiveClass ?? null;
        }

        CssClass = activeStateClass is not null
            ? CombineWithSpace(_class, activeStateClass)
            : _class;
    }
