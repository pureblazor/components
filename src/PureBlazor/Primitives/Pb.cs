...
/// <summary>
/// Similar to Shadcn's `cn` utility, this class helps manage a set of CSS classes
/// </summary>
public class Pb
{
    /// <summary>
    ...
    public string GenerateClass(params (string Category, string? Key)[] selections)
    {
        ...
        // always add the base classes if present
        if (!string.IsNullOrWhiteSpace(Base))
        {
            combined.Add(Base);
        }

        // for each (category, key) pair, look up and add the matching classes
        ...
        // remove duplicates so we don’t accidentally produce "bg-blue-500 bg-blue-500" or conflicting overrides.
